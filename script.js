try {
	...
	async function retry(t) {
		...
		n.text = i;
		let o = $(t).attr("class").split(/\s+/);
		o = o.filter((t => t.startsWith("id-"))), o = o.toString(), o = o.split("-")[2], n.voice = o;
		const l = [n];
		$(".remove").each((function() {
			...
			await $.ajaxQueue({
				...
				error: function(t) {
					$(`#${l[n].pid}`).replaceWith(`<button class="btn btn-warning  btn-md retry id-${l[n].did}-${l[n].voice}" id="${l[n].pid}" onclick="retry(this)">\n\n                <i class="fas fa-rotate mr-2"></i>\n\n</div></button>`), setTimeout((() => {
						sub_enbl(`#${l[n].did}`)
					}), 90), $("#send").replaceWith('<button class="btn btn-warning btn-lg btn-block w-50 btn-block p-3" onclick="retryAll()" id="send">\n\n            <i class="fas fa-retweet mr-2"></i>\n\n          </button>')
				},
				timeout: 15e4
	...
	function change_voice(t) {
		...
		let e = $(t).html();
		e = e.replace(/src="\w+.\w+"/g, `src="${{karishma:"james",james:"akash",akash:"karishma"}[n]}.png"`), $(t).html(e)
	}
	...
	function send() {
		...
		n.forEach((async (t, e) => {
			await $.ajaxQueue({
				...
				success: function(t) {
					"success" == t.message ? (l++, $(`#${t.pid}`).replaceWith(`<button class="btn btn-success  btn-md" id="${t.pid}">\n\n                <i class="fas fa-check-double mr-2"></i></div></button>`), $(`#${t.add}`).replaceWith(`<div class="container-audio" id="${t.add}"><audio controls controlsList="nodownload noplaybackrate novolume" id="${t.file}">\n\n                   <source src="${t.file}">\n\n                   Your browser dose not Support the audio Tag\n\n               </audio></div>`), ajex_complete(o, l, d), setTimeout((() => {
						back_to_voice(`#${t.pid}`, t.voice), sub_enbl(`#${t.did}`)
					}), 1500), $("#delete-all").removeClass("disabled")) : "error" == t.message && (d++, l++, $(`#${t.pid}`).replaceWith(`<button class="btn btn-warning  btn-md retry id-${t.did}-${t.voice}" id="${t.pid}" onclick="retry(this)">\n\n                <i class="fas fa-rotate mr-2"></i>\n\n</div></button>`), ajex_complete(o, l, d), setTimeout((() => {
						sub_enbl(`#${t.did}`)
					}), 90))
				},
				error: function(t) {
					d++, l++, $(`#${n[e].pid}`).replaceWith(`<button class="btn btn-warning  btn-md retry id-${n[e].did}-${n[e].voice}" id="${n[e].pid}" onclick="retry(this)">\n\n                <i class="fas fa-rotate mr-2"></i>\n\n</div></button>`), ajex_complete(o, l, d), setTimeout((() => {
						sub_enbl(`#${n[e].did}`)
					}), 90)
				},
				timeout: 15e4
